...
assert_build_info_key_contract() {
    local fixture
    fixture=$(mktemp)
    printf '%s\n' 'mecmcp_ref=v0.20.0' >"$fixture"
    has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.20.0' "$fixture" \
        || fail 'singular expected BUILD-INFO mecmcp key was rejected'
    printf '%s\n' 'mecmcp_ref=v0.7.3' >>"$fixture"
    if has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.20.0' "$fixture"; then
...
assert_exact_mecmcp_sbom_set() {
    ...
    valid=$(jq -nc '{
        ...
        components: [
            ...
            {name: "mecmcp-audit", version: "0.20.0"},
            {name: "mecmcp-auth", version: "0.20.0"},
            {name: "mecmcp-changeset", version: "0.20.0"},
            {name: "mecmcp-runtime", version: "0.20.0"},
            {name: "mecmcp-secret", version: "0.20.0"},
            {name: "mecmcp-server", version: "0.20.0"},
            {name: "mecmcp-transport", version: "0.20.0"}
        ]
    }')
...
[[ ${#builder_mecmcp_refs[@]} -eq 1 && ${builder_mecmcp_refs[0]} == 'v0.20.0' ]] \
...
require_logical_line \
    "has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.20.0' \"\$build_info\" || die 'BUILD-INFO mecmcp ref is invalid'" \
    "$installer"
require_logical_line \
    "has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.20.0' \"\$build_info\" || {" \
...
require_logical_line \
    "printf '%s\n' \"\$build_info\" | awk -F= -v expected='mecmcp_ref=v0.20.0' '\$1 == \"mecmcp_ref\" { count += 1; matches += (\$0 == expected) } END { exit !(count == 1 && matches == 1) }'" \
    "$ci"
sbom_validators=(
    scripts/build-lab-package.sh
    packaging/tests/package-smoke.sh
