...
assert_build_info_key_contract() {
    ...
    has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.7.3' "$fixture" \
        || fail 'singular expected BUILD-INFO mecmcp key was rejected'
    printf '%s\n' 'mecmcp_ref=v0.4.0' >>"$fixture"
    if has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.7.3' "$fixture"; then
...
[[ ${#builder_mecmcp_refs[@]} -eq 1 && ${builder_mecmcp_refs[0]} == 'v0.7.3' ]] \
...
require_logical_line \
    "has_single_exact_key mecmcp_ref 'mecmcp_ref=v0.7.3' \"\$build_info\" || die 'BUILD-INFO mecmcp ref is invalid'" \
    "$installer"
require_logical_line \
...
require_logical_line \
    "printf '%s\n' \"\$build_info\" | awk -F= -v expected='mecmcp_ref=v0.7.3' '\$1 == \"mecmcp_ref\" { count += 1; matches += (\$0 == expected) } END { exit !(count == 1 && matches == 1) }'" \
    "$ci"
sbom_validators=(
    scripts/build-lab-package.sh
