#!/usr/bin/env bash
# Build a local, commit-addressed release package. CycloneDX identifiers and
...
package_root="rustsdcmcp_0.0.1.${package_date}.${git_sha12}_amd64"
...
# which pins whichever version was current when it was written. Copying that
# file shipped every archive with download instructions for the previous
# release. Instructions for obtaining an archive are meaningless inside that
# archive; what a reader here needs is what this is and where to go next.
cat >"$stage_dir/README.md" <<EOF
# rustsdcmcp — package \`$package_root\`
...
- 1 vCPU, 512 MiB RAM, 512 MiB swap, 4 GiB disk for the minimum profile.
