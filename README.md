...
`rustsdcmcp` **v0.0.3** exposes **54 MCP tools**: 40 bounded read tools and 14
...
[`v0.0.3` release page](https://github.com/fastrevmd-lab/rustsdcmcp/releases/tag/v0.0.3).
...
gh release download v0.0.3 \
  ...
  --pattern 'rustsdcmcp_0.0.3.*_amd64.tar.gz*'
sha256sum -c rustsdcmcp_0.0.3.*_amd64.tar.gz.sha256
```
...
`ghcr.io/fastrevmd-lab/rustsdcmcp:0.0.3`.
...
```console
approved_commit=$(git rev-parse v0.0.3^{commit})
...
```bash
set -euo pipefail
mapfile -t archives < <(find . -maxdepth 1 -type f -name 'rustsdcmcp_0.0.3.*_amd64.tar.gz' -print)
test "${#archives[@]}" -eq 1
archive="${archives[0]}"
